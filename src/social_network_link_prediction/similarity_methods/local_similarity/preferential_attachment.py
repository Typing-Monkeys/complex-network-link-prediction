...
def preferential_attachment(G: nx.Graph, sum: bool = False) -> csr_matrix:
    ...
    Notes
    -----
    The idea of preferential attachment is applied to generate a growing scale-free network. 
    The term growing represents the incremental nature of nodes over time in the network. 
    The likelihood incrementing new connection associated with a node \\(x\\) is proportional to 
    \\(k_x\\), the degree of the node.

    This index shows the worst performance on most networks. 
    The **simplicity** (as it requires the least information for the score calculation)
    and the computational time of this metric are the main advantages. PA shows better results 
    if larger degree nodes are densely connected, and lower degree nodes are rarely connected. 
    
    In the above equation, summation can also be used instead of multiplication as an aggregate 
    function (`sum = True`).
    """
