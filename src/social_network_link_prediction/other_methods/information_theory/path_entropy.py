...
def path_entropy(G:nx.Graph, max_path:int = 3) -> csr_matrix:
    ...
    for elem in missing_edges:
        ...
        similarity_matrix[nodes_to_indexes_map[elem[0]],nodes_to_indexes_map[elem[1]]] = tmp
...
def simple_path_entropy(paths, G:nx.Graph) -> float: #paths è un generator ritornato dalla funzione nx.all_simple_paths()
    tmp = .0
    for path in paths:
